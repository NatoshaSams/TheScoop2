...
function createComment(url, request) {
  ...
  const response = {};
  //Checking for comment content and valid information in required areas.
    if (createComment && createComment.body && createComment.username && database.users[createComment.username] && createComment.articleId
        ...
        //Handling comment id and adding an id to items
        ...
        respone.body = {comment: newComment};
        response.status = 201;
      } else {
        response.status = 400;
      }
...
function updateComment(url, request) {
  ...
  const response = {};
  //If statement to check for existing user and id. Else to perform function.
  if (!savedComment) {
    response.status = 404;
...
function deleteComment(url) {
  ...
  const savedComment = database.comments[id];
  const response = {};
  //If statement to check if a comment exists. Else to perform function.
  if (!id || !savedComment) {
    response.status = 404;
  } else {
...
//Function to allow up vote of comment.
function upvoteComment(url, request) {
  const id = Number(url.split('/').filter(segment => segment) [1]);
  const username = request.body && request.body.username;
  let savedComment = database.comments[id];
  //If statement for existing comments and usernames.
...
//Function to allow down vote of comment.
